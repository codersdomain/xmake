...
function _make_headerunitflags(target, headerunit, bmifile)
    ...
    assert(module_headerflag, "compiler(emcc): does not support c++ header units!")
...
function populate_module_map(target, modules)
    local emcc_version = compiler_support.get_emcc_version(target)
    --local support_namedmodule = semver.compare(clang_version, "16.0") >= 0
    local support_namedmodule = semver.compare(emcc_version, "3.1.66") >= 0
