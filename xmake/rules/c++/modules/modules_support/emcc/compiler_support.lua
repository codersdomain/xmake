...
function _get_std_module_manifest_path(target)
    ...
    local emcc_path = path.directory(get_emcc_path(target))
    if print_module_manifest_flag then
        local compinst = target:compiler("cxx")
        local outdata, _ = try { function() return os.iorunv(compinst:program(), {"-std=c++23", "-stdlib=libc++", "--sysroot=" .. path.join(emcc_path, ".."), print_module_manifest_flag}, {envs = compinst:runenvs()}) end }
    ...
    local clang_lib_path = path.join(emcc_path, "..", "lib")
...
function load(target)
    ...
    local emcc_version = get_emcc_version(target)
    --if semver.compare(clang_version, "18") < 0 then
    if semver.compare(emcc_version, "3.1.66") < 0 then
...
function toolchain_includedirs(target)
    ...
    if includedirs == nil then
        includedirs = {}
        local emcc, toolname = target:tool("cxx")
        assert(toolname:startswith("emcc"))
        _get_toolchain_includedirs_for_stlheaders(target, includedirs, emcc)
        local cpplib = _get_cpplibrary_name(target)
        local runtime_flag
        if cpplib then
            if cpplib == "c++" then
        ...
        local _, result = try {function () return os.iorunv(emcc, table.join({"-E", "-Wp,-v", "-xc", os.nuldev()}, runtime_flag or {})) end}
        if result then
            for _, line in ipairs(result:split("\n", {plain = true})) do
                line = line:trim()
...
-- get clang path
function get_emcc_path(target)
    local emcc_path = _g.emcc_path
    if not emcc_path then
        local program, toolname = target:tool("cxx")
        if program and toolname:startswith("emcc") then
            local clang = find_tool(toolname, {program = program,
                envs = os.getenvs(), cachekey = "modules_support_clang_" .. toolname})
            if clang then
                emcc_path = clang.program
            end
        end
        emcc_path = emcc_path or false
        _g.emcc_path = emcc_path
    end
    return emcc_path or nil
end

-- get clang version
function get_emcc_version(target)
    local emcc_version = _g.emcc_version
    if not emcc_version then
        local program, toolname = target:tool("cxx")
        if program and toolname:startswith("emcc") then
            local clang = find_tool(toolname, {program = program, version = true,
                envs = os.getenvs(), cachekey = "modules_support_clang_" .. toolname})
            if clang then
                emcc_version = clang.version
            end
        end
        emcc_version = emcc_version or false
        _g.emcc_version = emcc_version
    end
    return emcc_version or nil
end
...
function get_clang_scan_deps(target)
    ...
    if not clang_scan_deps then
        ...
        if program and toolname:startswith("emcc") then
            ...
            -- Adjust path to look in `../bin` relative to emcc
            local parent_dir = path.normalize(path.join(dir, "..", "bin"))
            local scan_deps_program = path.join(parent_dir, "clang-scan-deps" .. extension)

            -- Check if the `clang-scan-deps` exists and is executable
            if scan_deps_program and os.isfile(scan_deps_program) then
                local result = find_tool("clang-scan-deps", {program = scan_deps_program, version = true})
                if result then
...
function get_modulesflag(target)
    ...
    if clangmodulesflag == nil and modulestsflag == nil then
        ...
        withoutflag = semver.compare(emcc_version, "3.1.66") >= 0
        assert(withoutflag or modulestsflag, "compiler(emcc): does not support c++ module!")
...
function get_builtinmodulemapflag(target)
    ...
    if builtinmodulemapflag == nil then
        ...
        if not target:is_plat("mingw") then
            ...
            assert(builtinmodulemapflag, "compiler(emcc): does not support c++ module!")
        end
...
function get_implicitmodulesflag(target)
    ...
    if implicitmodulesflag == nil then
        ...
        assert(implicitmodulesflag, "compiler(emcc): does not support c++ module!")
...
function get_implicitmodulemapsflag(target)
    ...
    if implicitmodulemapsflag == nil then
        ...
        assert(implicitmodulemapsflag, "compiler(emcc): does not support c++ module!")
...
function get_noimplicitmodulemapsflag(target)
    ...
    if noimplicitmodulemapsflag == nil then
        ...
        assert(noimplicitmodulemapsflag, "compiler(emcc): does not support c++ module!")
...
function get_prebuiltmodulepathflag(target)
    ...
    if prebuiltmodulepathflag == nil then
        ...
        assert(prebuiltmodulepathflag, "compiler(emcc): does not support c++ module!")
...
function get_modulecachepathflag(target)
    ...
    if modulecachepathflag == nil then
        ...
        assert(modulecachepathflag, "compiler(emcc): does not support c++ module!")
...
function get_modulefileflag(target)
    ...
    if modulefileflag == nil then
        ...
        assert(modulefileflag, "compiler(emcc): does not support c++ module!")
...
function has_clangscandepssupport(target)
    ...
    if support_clangscandeps == nil then
        ...
        if clangscandeps and emcc_version and semver.compare(emcc_version, "3.1.66") >= 0 then
...
function get_keepsystemincludesflag(target)
    ...
    if keepsystemincludesflag == nil then
        ...
        if compinst:has_flags("-E -fkeep-system-includes", "cxxflags", {flagskey = "clang_keep_system_includes", tryrun = true}) and
            ...
            semver.compare(emcc_version, "3.1.66") >= 0 then
